...
* Terminal (known as Command Prompt on Windows) is where you start the rails server and run commands.
...
CTRL-C to exit the server in Terminal.
...
Now we generate the needed code for picture handling. In the Terminal run:

{% highlight sh %}
bundle
{% endhighlight %}

**At this point you need to restart the Rails server process in Terminal.** This is needed for the app to load the added gem.
