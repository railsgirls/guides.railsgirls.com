...
### *3c.* If your OS X version is 10.10:

Mac OS X comes with a “system Ruby” pre-installed. Mac OS X Yosemite includes Ruby 2.0.0p481 which is not the newest version. You should update to Ruby 2.1.3 by using [RVM](https://rvm.io/), the Ruby Version Manager. First install RVM:
...
Check for [the current version of Rails](http://rubygems.org/gems/rails).

Now normally, you'd install Rails into what's called *the global gemset*. Instead, we'll make a gemset just for the current stable release:

{% highlight sh %}
rvm use ruby-2.1.3@rails4.1 --create
{% endhighlight %}

And then, if you want the most recent stable release for your project:
...
... to check if you are actually using the most current Rails version.
...
Download [RailsInstaller](https://github.com/railsinstaller/railsinstaller-windows/releases/download/3.0.0-alpha.2/railsinstaller-3.0.0.exe) and run it. Click through the installer using the default options.
