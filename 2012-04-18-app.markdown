...
* Serviço na nuvem (por exemplo: nitrous): Faça login na sua conta, inicie sua interface e vá para a IDE (ver [installation guide](/install#using-a-cloud-service) para detalhes). Em geral o terminal está localizado na parte inferior da janela do navegador.
...
{% endhighlight %}
    ...
    <div>
<p>Rodando o comando  <code>ls</code> no terminal você poderá constatar que um diretório denominado  <code>projects</code> foi criado. Você verá o diretório <code>projects</code> na saida do terminal. A seguir você deverá ir para o diretório  <code>projects</code> executando o seguinte comando:</p>
    </div>
...
Notar que nesta nova janela o prompt de comando não é visível, pois você se encontra no servidor Rails e o prompt de comando tem o seguinte aspecto:
...
Abra <http://localhost:3000/ideas> no navegador. Para serviços na nuvem (por exemplo: nitrous) acrescente '/ideas' na url do preview (ver [installation guide](/install#using-a-cloud-service)).
...
**Coach:** Fale sobre caminhos e os detalhes na ordenação dos caminhos e seus relacionamentos com arquivos estáticos.
