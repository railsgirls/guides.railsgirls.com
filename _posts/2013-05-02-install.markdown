...
After installing, fix some problems related railsinstaller-3.3.0.exe. Follow descriptions are in case of installing to `C:\RailsInstaller` folder.

Open `C:\RailsInstaller\Ruby2.3.0\bin\bundle.bat` file with editor and remove all of `C:\Users\emachnic\GitRepos\railsinstaller-windows\stage\Ruby2.3.0\bin\` words. The fixed file is follow.
...
Next, open `C:\RailsInstaller\Ruby2.3.0\bin\rails.bat` file and fix same as above. The fixed file is follow.
